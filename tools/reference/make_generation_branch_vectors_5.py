"""The seventh file of synthtwin's generation reference vectors.

This is the SEVENTH entry point of one oracle, not a seventh oracle.  It asks
`make_generation_reference_vectors.py` for the cases the final pass over
the close of stage 2 added to method section G14.3 -- a whole number a
column writes two ways held by two strata, by the fill and by the merge
(plan P4-D193) -- which live in a file of their own only because the
sixth file stands within a few kilobytes of the provenance manifest's
250000-byte cap (G14.2).  Everything the second entry point says holds for this one word
...
Usage:  python3 make_generation_branch_vectors_5.py --seed 0 --out <path>
...
def main(argv=None):
    """Write the seventh case set to the path given by --out.
    ...
    return oracle["main"](
        sys.argv[1:] if argv is None else argv, part=oracle["FIFTH_BRANCH_PART"]
    )
