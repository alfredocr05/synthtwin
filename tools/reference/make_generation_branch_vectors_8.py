...
`make_generation_reference_vectors.py` for the six cases stage 3's tail
rule adds to method section G14.3 (landing 3.3, plans P4-D321 to
P4-D327): the tail reading of G5.3b and the two derived ends
(`tail_shape_ends`), the listed tail of G5.3e and the counts solved for
it (`tail_listed_counts`), a published heaped end (`tail_heaped_end`),
the sign rule of G5.5a on a derived end (`tail_sign_clamped`), the
moment ladder of G5.3c (`tail_moment_ladder`) and the made-up ramp of
G5.3d (`tail_made_up_ramp`).

**Why they are a tenth file.**  Plan P4-D295 draws the line: an entry
