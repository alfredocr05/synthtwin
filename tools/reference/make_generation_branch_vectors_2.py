"""The third file of synthtwin's generation reference vectors.

This is the THIRD entry point of one oracle, not a third oracle.  It asks
`make_generation_reference_vectors.py` for the cases the carried landings
2b.2, 2b.3 and 2b.4 added to method section G14.3, which live in a file
of their own only because with them the second file would pass the
provenance manifest's 250000-byte cap (G14.2).  Everything below, written
for the second entry point, holds for this one word for word.

The second entry point's account follows.  The
...
Usage:  python3 make_generation_branch_vectors_2.py --seed 0 --out <path>
...
def main(argv=None):
    """Write the third case set to the path given by --out.
    ...
    return oracle["main"](
        sys.argv[1:] if argv is None else argv, part=oracle["SECOND_BRANCH_PART"]
    )
