...
`make_generation_reference_vectors.py` for the six cases the carried
numbers pass of 2026-09-18 and its repair pass of 2026-09-19 added to
method section G14.3: G6.5a's fill of a sign band whose own grid has no
spare point (`saturated_band`), the published mode on the stratum its
count sizes (`mode_held`, plan P4-D267), G8.3a's dressing and anchors
(`held_back_dressed` and `held_back_anchored`, plan P4-D268), G6.5a's push
of a collision the walks leave along its band to the nearest free point
(`pushed_along_band`), and G6.5a's column-wide fill of a grid with no
spare point (plans P4-D147 and P4-D176) on a column where the band fill
and the push both stand aside (`saturated_grid_alone`), so that each of
the three statements of the fill is held up by a case of its own.
...
numbers pass of 2026-09-18 built its four cases into the eighth, which
left it at 235440 bytes, past that line, so its repair pass opened this
file for its own two.  When the carried passes were integrated, the eighth
also held the carried date items' three cases and the two of the readings
of an absorbed count, and with all nine it would have passed the
250000-byte cap, so the numbers pass's four moved here whole.  No case was
dropped, no proof was shortened and the cap was not raised.  Everything
the second entry point says holds for this one word for word: the
...
**WHERE THE NEXT CASE GOES** (plan P4-D295): in the EIGHTH file, whose
output stands under 200000 bytes after the four moved here, until it
passes that line; this file's own output already stands past it.  The
first three files are full and take no case.
