...
- **Checks.** CI runs lint (`ruff`), types (`mypy --strict` on `src/`),
  ...
  aggregate check named `gate`. From Phase 3's visibility flip the
  branch ruleset makes `gate` the required context: pull requests
  only, and self-merge only after `gate` is green (see SECURITY.md's
