...
| suite | 4,376 collected on this worktree with the WRAPPER SET (landing L14, plan P4-D36), EVERY WRAPPER'S OWN NUMBERS (plan P4-D37, owner ruling 2026-09-08) and review rounds 1, 2 and 3 answered in it; `1 failed, 4324 passed, 51 skipped in 1009.82s (0:16:49)` verbatim at this count, the one failure being THIS LINE's own guard reading the count before it was moved; every other test green. **TWENTY-ONE new tests** since 4,364, all in `tests/test_p4d4_affixed_role.py`. **ROUND 3 RETURNED FIVE BLOCKERS AND ALL FIVE ARE CLOSED**: each wrapper's block is checked and listed, the commonest wrapper's residuals are counts that close (AF16), the block's pair must be the commonest wrapper (AF17), a flush trailing letter needs `--measurement` because width cannot tell a flagged reading from a code register, the compound rule stands aside where the affix rule takes the column (so a flagged column reaches ONE role at every draw, 40 of 40 where it was 30 of 40), and every wrapper owes its own records on the twin's report (30 where 13 were written). |
