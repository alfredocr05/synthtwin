...
| suite | 4,377 collected on this worktree with the WRAPPER SET (landing L14, plan P4-D36), EVERY WRAPPER'S OWN NUMBERS (plan P4-D37) and review rounds 1 to 5 answered in it; `4326 passed, 51 skipped in 1013.14s (0:16:53)` verbatim — **fully green** — with `ruff check .` and `mypy --strict src/` both clean. **ROUND 5 RETURNED SIX ITEMS**: three closed — the producer chose its commonest wrapper from what PROPOSED it while AF17 checks what WEARS it, so it could write a file its own loader refused; the wrapper-set report line claimed to check counts it does not; and the contract gave two population rules at once. **THREE ARE CARRIED into R-P4-157**: the compound rule runs BEFORE the affix rule, so neither the narrowed suffix guard nor `--measurement` reaches a column that route takes — an undeclared register of procedure codes gets a distribution through it, and the 30/10 tie is 30/10 declared or not. |
