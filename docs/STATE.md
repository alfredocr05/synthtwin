...
| suite | 4,378 collected; `4327 passed, 51 skipped in 1031.20s (0:17:11)` verbatim, fully green, with `ruff check .`, `mypy --strict src/`, the decontamination scan and the signed attestation all clean. **ROUND 6 RETURNED EIGHT ITEMS**: five closed — the variant census named a path no description carries (`...numbers.numeric.field_widths`); `--measurement` could not rescue a genuine PREFIX unit, so a person answered the question and still got nothing; an absent measured wrapper DROPPED its 41 quantitative obligations instead of gating them WITHHELD; a wrapper's layout deviations were written twice, once under a key an affixed block does not carry; and the contract said the numeric block holds SIXTEEN keys where `NUMERIC_KEYS` holds twenty-five, a number stale since `kurtosis`, `mode`, `percentiles_between`, `field_widths`, `value_histogram`, `empty_bins` and `empty_edges` were added. **THREE CARRIED**: per-wrapper G12.8 corner state, the rear-letter code register reaching the compound route (both R-P4-157), and the attestation re-signing, which is an owner action. **AND CI IS GREEN ON EVERY CELL** at `7be7df7` (run 34381013241): all twenty jobs succeeded — `gate`, `build`, `decontam`, `lint`, `types`, `minimums`, `offline-static`, `provenance`, `sensitive-paths`, and all eleven test cells including **all five Windows ones**. No Phase 4 commit had ever run CI before 2026-09-09, and the first run failed every Windows cell on one unpinned read. Acceptance criterion 10's CI clause is met on this tree. |
...
* **THE HISTOGRAM.** The owner's worry was named exactly: "moments and
  ...
  R-P4-49 is **CLOSED**, by the owner's ruling of 2026-09-09 on this
  measurement (amendment A-P4-58): the shape it was opened for is
  reproduced, and holding the twin to bin counts would allot cells by
  the histogram where the ladder's runs allot them. What remains is a placement detail, not a smooth twin.
