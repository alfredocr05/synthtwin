...
| suite | 4,453 collected at landing L19 (2026-09-10), after review round 1; `4402 passed, 51 skipped in 1163.59s (0:19:23)` verbatim, fully green, with `ruff`, `mypy --strict`, the offline import scan, the provenance check, the decontamination scan, the signed attestation and the disposition seal all clean. **Ten new tests**: five in `tests/test_p4r70_the_report_says_what_the_twin_carries.py` and five in `tests/test_p4r152_one_page_one_answer.py`, of which two were written against a repair that was wrong in the quiet direction before the demonstration report caught it. Three residuals closed: R-P4-70, R-P4-152 and R-P4-72's NF35 half. The frozen branch vectors were REGENERATED by their oracle and the provenance manifest moved with them. |
| suite, before this landing | 4,442 collected at landing L17b (2026-09-10), after review round 1; `4391 passed, 51 skipped in 1158.65s (0:19:18)` verbatim, fully green |
...
* **LANDING L19 IS BUILT (2026-09-10): THE TWIN'S OWN REPORT STOPS
  CONTRADICTING ITSELF AND THE FILE BESIDE IT.** Three residuals, all
  of them sentences a person acts on.

  **R-P4-70 — the report's disclosure sentence pointed the wrong way.**
  It told every reader that the twin writes every absent cell empty, so
  their own `NA` or `-9.99` stayed behind in the description. Measured:
  a declared `-9.99` column comes out with 0 blank cells and 20 wearing
  `-9.99`. The sentence was true of contract version 5 and stopped
  being true when P4-D6.1 made the twin write each published spelling
  at its count. The split is now asked of
  `generation.spellings_the_twin_reproduces` -- the WRITE rule itself,
  so the report cannot disagree with the file it describes -- and every
  published spelling is marked one by one. The heading's new paragraph
  prints only where some spelling of THAT description travels, so a
  page cannot announce carried spellings over a table that has none.

  **R-P4-152 — one number was both 'not met' and 'inside its range'.**
  `_not_settled_by_a_bound` drops a deviation for a fact whose
  approximation landed inside its bound, applied once where the two
  kinds of note are assembled rather than in each of the six roles that
  recount a count with a window. **The first version of it was wrong in
  the quiet direction and the demonstration report caught it**: dropping
  on `inside` alone also removed `recorded_on`'s counts, which publish
  84 against a twin holding 224 inside a bound of 106 to 240 -- the
  method's promise, nowhere near the description. None of these bounds
  is a margin around the published value, so the filter asks
  `covers_published` too.

  **R-P4-72's NF35 half — the advice named the declaration that throws
  the distribution away.** A column of `12 mg` was told "if these are
  codes, run again with --identifier", which publishes no value of the
  column at all. `--code` is named first now. **Four copies of one
  sentence moved in one commit**, which is why it waited: the contract
  clause, the producer's rendering, the LOADER's `AFFIXED_REMARK_PARTS`
  (AF-R refuses a block whose remark is not that sentence, so a
  producer change alone makes every affixed description unloadable) and
  the independent oracle. The frozen vector was REGENERATED, not
  edited; its proof layer proved the same 544 numbers; exactly one
  string moved in it; the provenance manifest moved with it.

  **R-P4-61 is NOT closed and is carried**: the generator and the
  validator print moment windows differing in their last two digits,
  and closing it means stating the arithmetic operation by operation in
  G12.3 and making both implementations follow that text -- a
  specification landing of its own, which the one-week close does not
  have room for.

  **REVIEW ROUND 1 RETURNED RATIFY-WITH-CONDITIONS, one repaired and
  two carried, and the repair was a REGRESSION THIS LANDING
  INTRODUCED.** Repairing NF35's sentence changed which fragments AF-R
  looks for, and AF-R looked for each of them in the WHOLE remark --
  including inside the block's own affix pair, which is the person's
  DATA and may spell anything. A column of `12 run the command again
  with --code NAME` publishes that text as its suffix, so the advice
  fragment was found inside the quotation, before the advice; the order
  test failed; and **the loader refused a description its own profiler
  had just written**, telling the person to make it again, which
  reproduced it exactly. The opening and the clause are POSITIONS now
  and every fragment after them is looked for after them, which is a
  place no cell can reach. Measured on three shapes -- prefix, suffix
  and both -- all round trip.

  **Carried by name:** R-P4-168 (AF-R reads fragments in order and not
  the text between them, so a remark saying "do NOT run the command
  again with --code NAME" is accepted; pre-existing), R-P4-169 (the
  heading's walk resolves the decimal-comma declaration per column, so
  a table of C declared columns costs C(C+1)/2 comparisons).
