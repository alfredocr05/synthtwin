...
| suite | 4,379 collected; `4327 passed, 51 skipped in 1025.76s (0:17:05)` verbatim, fully green. **ROUND 7 RETURNED FOUR ITEMS AND ALL FOUR ARE CLOSED**: a wrapper was measured against the wrong population wherever the checked file's DOMINANCE was flipped — both sides publish their commonest wrapper at the block's root, so a description of 120 kilograms and 80 pounds read against a file of 120 pounds and 80 kilograms compared kilograms to pounds and called it HELD, while the pound facts came back WITHHELD because nothing looked at the root for them; the common wrapper's `n_distinct_values` vanished from BOTH report halves, because the check side asked its envelope over the core view and the listing side over the outer column; every subset view kept the COLUMN's missing count, so a style ceiling drawn for 200 rows covered a wrapper whose population is 100; and one recorded measurement was arithmetically impossible — 240 readings averaging 14.514 beside sixty markers averaging 29.5 pool to **17.511**, not the 17.46 recorded beside it. |
