...
| suite | 4,317 collected on the merged tree with the integer-grid landing and its first five review rounds in it (52 skipped on another machine: that count moves, the collected count does not). **SEVEN new tests**: `tests/test_p4r56_grid_separation.py` goes from 9 to 15 -- the integer grid, its refusal on a column that is not whole-valued, the contrast showing both halves of the rule are needed, the inclusive share endpoint a binary step used to refuse, the sixty-four GRID steps a binary step used to overrun, and the round trip that keeps the walk from spelling a text the column already holds -- and one in `tests/test_p2c2f3_style_invention.py` pins that a small whole-number column now reaches its count |
