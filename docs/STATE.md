...
| suite | 4,378 collected; `4327 passed, 51 skipped in 1085.44s (0:18:05)` verbatim, fully green, with `ruff check .` and `mypy --strict src/` clean. **AND IT HAS NOW RUN ON CI**, which no Phase 4 commit had ever done: every Ubuntu and macOS cell passed and **all five WINDOWS cells failed**, on one defect — `test_method_citations_resolve` read a governing document with no encoding, so Windows used cp1252 and stopped on byte 0x9d. The PRODUCT was never affected: every read in `src/synthtwin/` already pinned UTF-8. Twenty-seven reads and writes across the guards and tools did not, and every one of them is pinned now. **AND THREE OF THEM COULD NOT BE**: `check.py`, `surfaces.py` and `verify_attestation.py` are bound by the SIGNED attestation, and pinning their reads changed the scanner-tree digest — `verify_attestation.py` refused the tree with DIGEST DRIFT, which is that control working exactly as written. Re-signing needs the maintainer's key and the private coverage battery, so the edit was reverted and the three unpinned reads stand. They read `magic.txt` and the manifest, which are ASCII, so nothing fails today; a non-ASCII byte in either would break the scanner on Windows. **This is an owner action**: re-run the private battery and re-sign, or rule that the scanner stays as it is. |
