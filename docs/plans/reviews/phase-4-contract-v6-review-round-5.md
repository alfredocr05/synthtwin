...
| P4-X5-F10 | REPAIRED. C6-MIG-B is specified rather than described: exact search paths, exclusions named with reasons (the older contract documents, the plans and the changelog record what was true when written), the literal word, three exact phrases, an eighty-character window, and an expected-hit manifest of twelve lines across nine files — computed by running the search, not by listing from memory. |
