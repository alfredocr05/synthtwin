...
**The asking.** A column of figures grouped in threes whose values are all the same number of figures -- `123 456 789` -- is asked about as an identifier may be, and a column whose every value has three figures after its point -- `12.345` -- is asked whether the point is a mark between thousands, with `--decimal-comma` offered as an answer. The reading of `12.345` does not change (A-P4-56, A-P4-58).

**Frozen.** The independent oracle mirrors every rule in the same commit, and three frozen cases pin them with a mutant each: a comma beside signed decimals and zeros spent past order nought, a point on a declared decimal-comma column beside absent cells, and a space with accounting brackets. A fourth case was not added: it carried the branch vectors past the provenance guard's byte cap. Owner decisions 4 and 5 are untouched. The validator still does not hold `datetime_separators`, which rests on the reason withdrawn here for the thousands mark, and that stays carried by name.
