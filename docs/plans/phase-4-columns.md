...
**What landing 2b.12 carried, re-measured and CLOSED.** A code column at a floor of twenty, declared: the base twin's mean cell length is 4.004 against the real column's 6.126 — a third SHORT, not long, the walk taking the shortest room first — with the length mix collapsed from `{4: 233, 7: 567}` to `{4: 799, 7: 1}`. On this tree it is `{4: 233, 7: 567}`, exactly, ratio 1.000. The same column left UNDECLARED was already exact at base, through `shape_forms`; it is the DECLARED path that had no length census, and this key is it.
