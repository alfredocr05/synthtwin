...
`group_separator` is the mark a column writes between thousands, published on every numeric block as one character or the empty string; its class and its obligation are set by P4-D40 (landing 2b.2). It repairs a defect in which a charge written with a comma came back from the twin without one, so that code developed on the twin silently discarded every charge over a thousand when it met the real table: a mean of 412 against a true 918, with no error raised.
...
Carried by name at the stage 2 audit and repaired by landing 2b.2 (P4-D40): accounting brackets and a leading plus on a decimal, which were not published, and a column grouped with a space or an apostrophe, which was read as free text. Still carried: a free-text shape holding a comma is never written.

### P4-D39 The mark inside a moment, and a date held at midnight (stage 2, 2026-09-14)

`datetime_separators` is REPORT-ONLY. It counts the mark a moment writes between its day and its clock — `upper_t` for the letter T, `space`, `lower_t` for the letter t — on every datetime block, floor-governed exactly as `utc_offsets` is, with the names below the floor pooled under `(withheld)`. It is `{}` where the resolution is not `datetime`; the whole-date cells of an `iso-mixed` column are not counted, and a slashed stamp counts as `space`. It repairs a defect in which a stamp written with a space came back from the twin with a T, so that code splitting on that character worked on every twin row and failed on every row of the real table.

`all_at_midnight` is REPORT-ONLY. It marks a column whose every moment stands at midnight: `true` only on a datetime column read at `local` whose parsed cells reach the floor and every one names clock 00:00, seconds 00 and a fraction of zeros, a whole-date cell of `iso-mixed` counting as midnight. It repairs a defect in which a date stored as the date plus 00:00:00 came back with invented times of day: 800 of 800 real rows at midnight became 2 of 800. Contract invariants D12 to D14 hold both keys, and a `true` whose ends or rungs are not at 00:00:00 is refused. D12 also bounds the pool (the stage 2 audit, 2026-09-14): a `(withheld)` count may be no larger than the floor less one, times the number of permitted marks the census leaves unnamed, the permitted marks being the three names, or only `space` for a slashed format.

The generator spends the census over the parsed ranks by a smooth weighted rotation that draws no word: each rank adds every name's count to that name's credit, takes the name with the most credit, the first in sorted order on a tie, and subtracts the parsed total from it. Ranks the named counts do not cover — the withheld pool, and the whole-date cells of `iso-mixed` — are added to the commonest named mark, or written with T where no name is published, and a pooled mark is named in the report as a deviation. The rotation spreads each mark across the date range, so the twin invents no link between a moment's date and its spelling. An `all_at_midnight` column is generated in whole days, exactly as a column of dates is, and each cell is written as its day with a midnight clock at the column's precision and fraction width; the report recounts the midnight cells it wrote and names any that are not. The validator reads such a column in day units. Published instants keep their canonical space-separated text; only the twin's cells carry the mark. The independent oracle mirrors these rules in the same commit, as amendment A-P4-59 requires.

A cell whose written spelling the table declares absent (the stage 2 audit, 2026-09-14) is offered, in order, each mark the census names, in sorted order, and then the other common form, a space for T or t and a T for a space; it takes the first offer whose spelling is not absent. Absent spellings are drawn from every column and matched whatever the case of their letters. Where every offer is absent the value keeps its spelling and the report names it as a deviation of `n_present`. Each changed cell then hands its owed mark to the first rank, in rank order, that was allocated the mark the changed cell now wears, still wears it, was not touched before, and whose new spelling is not absent; a repeated spelling is allowed, no change may leave the column one spelling fewer nor one value fewer once case is ignored, and the walk is linear. Where every offer is absent, an interior rank of a column counted in whole units steps to the nearest unit whose spelling is not absent, inside its own window first (stage 2 confirmation review, 2026-09-15). A twin whose values come to two or fewer, case ignored, where the description counts three or more is named as a deviation of `n_distinct_folded`. A shortfall is named as a deviation of `datetime_separators`. A column of moments whose absent cells were pooled below the group size, in a table that declared missing values, carries a remark in the report that the twin cannot avoid an absent spelling the description does not publish; a remark and not a deviation, because no published fact was missed.

The twin report's sentence on the marks is the one true of the column. Where every mark was pooled, the twin writes a T. Where a remainder was pooled, or the column is `iso-mixed`, a value whose mark was not named takes the commonest mark. Otherwise each mark is written as often as the description records it. The resolution-mix deviation says code reading the cells as dates reads the same days only where `all_at_midnight` holds.
...
### P4-D40 How a number's grouping and sign are written (landing 2b.2, 2026-09-15)
