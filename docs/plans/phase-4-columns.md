...
**No generator rule moves, measured rather than inherited.** The twin of a padded wide column already writes canonical text: 800 of 800 padded wide cells canonical on every run, before this landing and after it. Ten shapes this landing does not touch were generated from a base-source copy and from this worktree and compared by sha256 of the twin bytes — identical on all ten — and `tools/provenance/check_provenance.py` passes with every frozen vector byte-identical.
...
**What it does NOT close, named rather than buried.** The oracle carries no frozen case that exercises this word, and the measurement states that precisely rather than loosely. Every block of every committed vectors file publishes `wide_runs: "none"`, and `tools/reference/make_generation_reference_vectors.py` writes that word as a DEFAULT on any numeric block that lacks one rather than reading it off the rule — so the word is not mirrored there at all. One frozen case does hold wide cells: `numeric_pooled_spelling` writes `100000000000000000000` six times, `plain` and canonical. **No committed case anywhere holds a PADDED wide cell** — measured, nought across all three files — so no committed byte can move whichever way this rule is read, and the padded reading this decision adds is unreachable from the oracle as it stands. That gap is pre-existing, this landing neither created nor widened it, and the rule is held meanwhile by the round-trip gate in `tests/test_landing_2b16_number_readings.py` and by four mutation checks that turn it red in both directions — two withdrawing the reading, two withdrawing the admission and the room.
