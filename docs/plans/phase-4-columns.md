...
The generator writes the mark into the plain, leading-plus and decimal forms at leading-zero order zero, and into nothing else. The independent oracle mirrors that rule in the same commit, as amendment A-P4-59 requires, and a test holds three separately written groupers to agreeing. The class is the weaker one because a file is read the same way with or without the mark. The validator offers a grouped spelling of every value whatever mark the description publishes, so a real table whose commas were not proven is never MISSED for them; it does not hold a twin to carrying the mark, and that is carried by name. The quality report lists the mark on every numeric-family column, as the field-width census beside it is listed, so the fact is named and never silent.
...
Also carried by name (the stage 2 audit, 2026-09-14). Accounting brackets and a leading plus on a decimal are not published number styles, so the twin writes a minus for the brackets and drops the plus, and a real table written with brackets misses its own `styles.spelled` check; this predates stage 2. A free-text shape holding a comma is never written. A column grouped with a space or an apostrophe is still read as free text.
