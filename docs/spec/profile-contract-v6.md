...
| `wide_runs` | string | `"none"`, `"canonical"` or `"respelled"` | whether the column's WIDE runs of figures are the text their own values write. A cell is such a run where it is written `plain`, carries no point and no exponent, and its value is at or past 2**53 in either direction — the bound past which more than one run of figures reads back as one double, so that "a spelling of its own value" stops naming a single text. `"none"` where the column wrote no such cell; `"canonical"` where it wrote some and every one of them is the figures its own value writes; `"respelled"` where it wrote some and at least one is not. It carries no count, so no floor governs it and it never pools: the word is a fact about the column's WRITER, naming no cell, no count and no figure (WR1; landing 2b.13) | EXACT-OBSERVABLE (plan P4-D90) |
...
| WR1 | `wide_runs` is one of `"none"`, `"canonical"` and `"respelled"`, and anything but `"none"` only where the `plain` count of `numeric_styles` plus its `(withheld)` remainder leaves room for at least one cell. It carries no count and no floor governs it | yes |
