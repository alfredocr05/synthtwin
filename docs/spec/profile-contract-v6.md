...
| `wide_runs` | string | `"none"`, `"canonical"` or `"respelled"` | whether the column's WIDE runs of figures are the text their own values write. A cell is such a run where its CORE — the cell with any surrounding space, accounting brackets, minus sign of the character tables and thousands marks taken off, exactly as section 7.5.4 reads a form off it — is a point-free run of base-ten figures with or without a sign, its form is `plain` or `leading_plus`, and its value is at or past 2**53 in either direction — the bound past which more than one run of figures reads back as one double, so that "a spelling of its own value" stops naming a single text. A cell written `leading_zero` is NOT one: its figures are not its value's figures by construction, and which of them are the pad is the width census's question, not this one's. `"none"` where fewer such cells than `small_cell_floor` were written; `"canonical"` where at least that many were and every one of them is the figures its own value writes; `"respelled"` where at least that many were and at least one is not. It carries no count and never pools, but the floor holds it as NS1 holds the notation beside it, because the word names the FORM of the cells it is about and below the floor the forms map has pooled that form away: the word is a fact about the column's WRITER, naming no cell, no count and no figure (WR1; landing 2b.13, repaired by plan P4-D91) | EXACT-OBSERVABLE (plan P4-D90) |
...
Twenty-seven keys. Every one is present in every block of these two
...
| `group_separator` | string | as on `count` and `continuous`, and `"."` only where the column is named in `settings.forced_decimal_commas` | the mark between thousands the CORES were written with, under the same evidence rule as on `count` and `continuous` (GS1). Since landing 2b.16 the declaration reaches this role's cores, so the point stands here under exactly the condition it stands under on a plain numeric column and never otherwise. **A NAMED LIMIT, measured rather than implied:** a declared column whose cores group their thousands with a point — `645.121,62 EUR` — publishes `""` today, because the core is read in the declared grammar, which drops the point before the mark evidence is weighed; 800 such cells at floor eleven publish no mark, and their twin writes the value ungrouped, `842480,87 EUR`, at exit 0 with nothing missed. The values are right and the grouping is lost, which is the carried half of this row | EXACT-OBSERVABLE (plan P4-D41, P4-D106) |
...
| `affix_variants[].numbers` | object | AF13 | the twenty-seven keys of a `count` or `continuous` block, read over this wrapper's cores and echoing its `count` in `n_rows` | as on `count` and `continuous` |

**The block is fifty-nine keys**: the twenty-two universal keys of
section 5.1 and the thirty-seven above — a `count` block's twenty-seven
additions plus this role's own ten. The matrix of section 6.11 marks
exactly those thirty-seven cells in its `afx` column. There is no
...
**Where the census OWES THIS STAND-IN NO FORM the stand-in is
`group-1`, `group-2`, … exactly as before.** Three ways that happens,
...
does not say every suppressed level wears a form**, and a reader must
...
| WR1 | `wide_runs` is one of `"none"`, `"canonical"` and `"respelled"`, and anything but `"none"` only where the point-free counts of `numeric_styles` — `plain` and `leading_plus` — plus its `(withheld)` remainder leave room for at least max(1, `small_cell_floor`) cells. It carries no count and never pools, and the floor holds it for the reason NS1 holds `negative_form` (plan P4-D91) | yes |
