...
| `n_distinct`, `n_distinct_folded` | REPORT-ONLY, with the achieved count named beside the published one. **These two were disposed NOWHERE until 2026-08-31**: 9.2 sets them "per role group, in 9.3 to 9.7" and this role's table set neither, so a validator filed them under the disposition of another role entirely. They are report-only because the count of different CELLS is a CONSEQUENCE of the construction rather than a target: the pairing walk moves the last position to meet `part_agreements` and `part_above`, and how many different pairs that leaves is not aimed at. Measured through the real reader, producer, loader, generator and validator: a column whose cells REPEAT overshoots badly (80 different cells published, 210 held), and even an all-different column reaches the count at one seed and misses it by one at another, so an exact obligation would be met or missed by the seed. The generator's own report names the shortfall in words either way, so nothing is hidden by not checking it. Residual R-P4-40 names the description change that would make it reachable, and a later landing may raise this row |
