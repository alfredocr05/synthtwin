...
cells -- and a twin then missed `rows.order` (plan P4-D169); FD8 written header cells stand
...
count (`[h]`, `[mm]`) is (plan P4-D165). `formulas` counts the cells
...
**The disclosure rule every count here keeps** (plan P4-D160,
...
**A column mixing how its cells are stored is refused** (plan P4-D162,
...
spans (WB7). It fell at two rows AND two columns until plan P4-D166,
