...
| id | statement |
|---|---|
| RM1 | `resolution_mix` keys are exactly what the column's `format` permits: on a single-format column that member, on `iso-mixed` exactly `iso-date` and `iso-datetime`. The counts are exact and no floor governs them, and that is safe only because the joint READING is never taken where one of its two forms is written by fewer cells than max(2, `small_cell_floor`) — such a column is read as free text instead (plan P4-D279; 395 ISO dates beside one moment published `{"iso-date": 395, "iso-datetime": 1}` and named that record) |
