...
| `n_distinct`, `n_distinct_folded` | EXACT-OBSERVABLE where the published count lies inside G12.5's envelope on a column method G7.3's count pass applies to (`contract.datetime_counts_reachable`), which it reaches (plan P4-D192); APPROXIMATED everywhere else — the envelope is G12.5, and it is stated there that it need not contain the published count |
