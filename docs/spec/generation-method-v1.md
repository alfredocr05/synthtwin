...
4. otherwise the fitted shape read at the OUTERMOST ROW'S OWN SHARE,
   ...
   count is every numeric cell, the census says every cell is written
   `w` figures wide and a pad only adds zeros in front, so the published
   facts admit no value reaching `10**w` -- which is what they SAY and
...
**AND THE REPAIR OF THE ORACLE'S DERIVED END ADDS THREE** (stage 3's
review, verdict item 10). The oracle held step 4's obsolete order --
the one field width before the sign rule -- and no mark between
thousands, with every committed byte unchanged, because no case froze
