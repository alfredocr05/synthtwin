...
15. **The BLOCK-CLASS source accounting, reaching columns that
    ...
    And `missing_by_source` is EXACT-OBSERVABLE, so under C6-37 the
    twin WRITES those spellings at their published counts — **with
    C6-37's own named exception**: a spelling a JUDGED PASS put there,
    one reading as a stand-in number or as a calendar placeholder,
    stays blank in the twin (C6-38), and section 9's row carries the
...
columns publish labels, which is exactly what the `max` in C6-15 exists
to prevent.
