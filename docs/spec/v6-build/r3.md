...
not read it as a hole — C6-39 states that rule over the numeric pass,
...
exception the hole-spelling reproduction rule states (C6-37, with its
reason at C6-38).

**Invariant V4 (the order of `sentinel_verdicts` entries).** Entries
appear in this order, and the rule is exhaustive over the candidates
this format permits:
...
single entry; the group order therefore settles the mixed case that CAN
arise — numeric together with calendar — and states the rest for
completeness rather than leaving a reader to infer it. An ordering
stated only for the mixed case would leave a calendar-only block
ordered by nothing at all: ascending and descending would both conform,
and the same declared inputs would canonicalize to different bytes.
