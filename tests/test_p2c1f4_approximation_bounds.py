...
def test_every_approximated_fact_of_every_role_is_measured(
    ...
    for place in range(len(every_role.columns)):
        ...
        # A RUNG THE TAIL RULE WITHHOLDS IS MEASURED NOWHERE (stage 3,
        # contract TL1): the description publishes no number there, so
        # there is nothing to hold a twin to and the quality report
        # LISTS the rung instead. The inventory is the matrix's, less
        # the rungs this column's own block does not carry.
