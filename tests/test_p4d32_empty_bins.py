...
def test_a_column_whose_values_are_all_one_number_names_no_bin(
    ...
    # So the shape it still NAMES is asked at the floor where nothing is
    # withheld: read and described at a floor of ONE, the varying
    # position counts its values into GROUPS while the constant one
    # beside it names none. THE CENSUS ITSELF IS NOT THE QUESTION AT ANY
    # FLOOR SINCE LANDING 3.3: a tail block publishes `value_histogram:
    # {}` whatever the floor is (contract 6.7a), because the groups
    # between its two boundary rungs are where its shape is published --
    # and a floor of one is a tail block too, since a tail's own rows are
    # `max(small_cell_floor, 3)` and never fewer than three. What the
    # floor moves is how many groups there are: at one, where a group
    # needs only three rows, each of the thirty-two bins of the scale is
    # its own group; at eleven the same scale is carried by eight. The
    # constant position has no scale to group at either.
    ...
    assert len(at_one["parts"][1]["bin_groups"]) == 32, at_one["parts"][1]
    assert len(second["bin_groups"]) == 8, second["bin_groups"]
    assert at_one["parts"][0]["bin_groups"] == [], at_one["parts"][0]
