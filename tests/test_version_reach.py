...
ROUTES_THAT_RUN_SOURCE = {
    ...
    ("tools/reference/make_generation_branch_vectors_11.py", "runpy.run_path"): 1,
...
# so the seven items of the adversarial round are held to the back-solve
# that driver already holds rather than to a third copy of it. What it
# loads is the same committed file under tools/ the gate loads, which
# this guard reads directly, so the floor holds for it as it does for the
# other twenty-eight.
#
