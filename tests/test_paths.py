...
# ---------------------------------------------------------------------------
# Spy: resolution must never run on lexically rejected input
# ---------------------------------------------------------------------------


def test_resolve_is_never_invoked_on_lexical_rejection(
    ...
    for raw in URL_INPUTS + UNC_INPUTS + DEVICE_INPUTS:
        with pytest.raises(PathValidationError):
            validate_local_path(raw, purpose="input")
    assert calls == [], (
        "Path.resolve ran on lexically rejected input; D6.1 requires "
        "lexical checks before any filesystem call"
    )
