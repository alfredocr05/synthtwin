"""Plan P4-D86's gate: the twin is written the way its source file was.
...
def test_a_sort_column_holding_a_cell_of_spaces_publishes_no_order(
    ...
    # the spaces a writer put in an absent cell (plan P4-D74): the column
    # publishes `"   "` among its absent spellings, counts no blank cell,
    # and the twin writes the spaces back, which the published order
    # holds -- so FD7's own condition is met, the order is published and
    # kept, and both files still validate. Measured: exits 0 and 0, the
    # twin's first record opens with the three spaces.
    assert result["form"]["row_order"] == {
        "collation": "text", "column": 1, "direction": "ascending",
    }
    assert result["twin_form"]["row_order"] == result["form"]["row_order"]
    assert b"\n   ," in result["twin"]
    _held(result)
...
def test_a_mark_shaped_prefix_in_a_fallback_encoding_is_data(
    ...
    assert twin[:3] == b"\xef\xbb\xbf"
    # MERGED WITH LANDING 2b.8 (the integration of landings 2b.6 to
    # 2b.10, 2026-09-16): the ONE obligation this left, the published
    # shape form `@%%%%%` of the cell holding the stray byte, is now met
    # too, because landing 2b.8 settles a published form before the
    # lengths (plan P4-D75), so the twin writes all 119 cells the census
    # names. Measured: exit 3 with `forms.published.@%%%%%` missed on
    # landing 2b.10's tree, exit 0 with nothing missed on the merged one.
    checked = tmp_path / "mixed-twin"
    checked.mkdir(parents=True, exist_ok=True)
    (checked / "other.csv").write_bytes(twin)
    (checked / "checked").mkdir()
    assert _exit_of(
        [
            "validate", str(described), "--twin", str(checked / "other.csv"),
            "--out-dir", str(checked / "checked"), "--replace",
        ]
    ) == 0
    report = (checked / "checked" / "other-quality.txt").read_text(encoding="utf-8")
    # The twelve the review measured are among what is not missed, and
    # so is everything else: no line of the report says MISSED.
