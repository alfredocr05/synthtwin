...
def _independence_lines() -> "list[str]":
    ...
    return [
        ...
        "   IDENTIFIER column, the twin does repeat its identities in the",
        "   same pattern yours does -- how many of them appear once,",
        "   twice or eight times is a published fact of that column and",
        "   the twin holds it. What carries nothing further is WHICH",
        "   identity gets which count, and which rows share one: the rows",
        "   of a subject in the twin hold unrelated dates, readings and",
        "   outcomes. So anything that groups rows -- an average per",
        "   person, a repeated-measures model, the number of visits one",
        "   person had -- behaves differently on the twin than it will on",
        "   your table. The twin is faithful one row at a time, and a",
        "   twin of a repeated-measures table misdescribes the",
        "   subject-level truth even where every column of it is right on",
        "   its own.",
        "",
...
def _approximation_lines(twin: generation.Twin) -> "list[str]":
    ...
    lines = [
        ...
        "",
        "A RANGE IS NOT ALWAYS A MARGIN AROUND THE DESCRIPTION'S VALUE.",
        "It is what the method allows the twin's own value, worked out",
        "from the description and the size of your column. A few ranges",
        "are margins and are centred on the value beside them; most are",
        "not, and one of those can sit wholly above or wholly below it --",
        "most often at the top of a column of dates, where the twin's",
        "rung is read at a slightly different share of the column from",
        "the share the rung's name names. Where a range does not cover",
        "the description's value, this report says so on that fact's own",
        "lines. Either way 'inside the range' means only that the method",
...
def _mark_lines(facts: contract.DatetimeFacts) -> "list[str]":
    ...
    if not _named_census(facts) and facts.parser_family in contract.CLOCK_FORM_MEMBERS:
        ...
        # (landing 2b.3), and since plan P4-D220 a pool of marks is the
        # whole census, spent one value on each rarer mark and the rest on
        # a capital T; the sentence says that and no more: which of them
        # the table wrote, the description does not say. The sentence for
        # a pool beside a named mark went with the pool itself, which
        # contract D12 refuses.
        lines = [
            "  Which marks your table wrote between the day and the time of",
            "  day is held back, because at least one was written by too few",
            "  values to name. The twin writes a space and a lower-case t on",
            "  one value each, where it has values enough, and a capital T on",
            "  the rest, which need not be the marks your table used.",
        ]
    else:
        lines = [
