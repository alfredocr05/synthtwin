...
def _written_empty(column: "ColumnBlock") -> int:
    """How many of a column's absent cells the twin writes EMPTY (plan P4-D169).
...
class WorkbookColumn:
    ...
    # P4-D160). One of `dialect.SHEET_VALUE_CLASSES`.
...
def _value_class(value: object, where: str) -> "str | None":
    """A column's commonest value class, by name, or nothing (P4-D160)."""
    if value is None:
        return None
...
def _workbook_rules(
    ...
    # (plan P4-D160). Each census was held to the floor count by count,
    ...
    for name in form.sheet_names:
        ...
        # P4-D167), so a description naming two could only be written
    ...
    if form.empty_rows_inside is not None:
        ...
        # 2b.10, plan P4-D160). One such record inside a table names that
