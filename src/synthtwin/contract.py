...
# P4-D321). They travel together and a block carries both or neither: a
...
INVARIANTS = {
    ...
    # THE TAIL RULE (stage 3, plans P4-D321 to P4-D327). A block that
...
class NumericFacts:
    ...
    # THE TAIL RULE (stage 3, plan P4-D321). `tail_rule` is true on a
...
def _tail_facts(
    ...
    if block["low"] is None or block["high"] is None:
        ...
        return TailFacts(low=None, high=None)
    if percent is None:
        raise _broken(
            "TL3",
            where,
    ...
    return TailFacts(low=low, high=high)
