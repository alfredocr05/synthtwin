...
def _escaped(text: str) -> str:
    ...
    P4-D163). Every XML reader turns a literal carriage return into a
...
def number_spelling(text: str) -> str:
    ...
    numbers (plan P4-D162); one writing of it keeps the two in step.
    """
...
# (plan P4-D162), in the order a census is handed out in.
_TOLD_BY_SPELLING = (
...
def cell_classes(
    ...
    (plan P4-D162). The first writing handed the error, boolean and text
    ...
    P4-D160). A cell no published count claims goes to the column's
...
def cell_format_kinds(
    ...
    A WITHHELD COUNT IS NOT A LICENCE FOR PLAIN (plan P4-D162). Where
    ...
    back `plain 20` and missed `workbook.format-kinds`. Such a cell
    takes the kind of the column's published code where that kind was
    withheld, then a withheld kind, and plain only where plain was not
    denied.
    """
    written: "list[int]" = []
    absent = 0
...
def _sheet_part(
    ...
    if write_header:
        ...
        for index in range(width):
            ...
            # P4-D161). Its column is named `Unnamed: N` for its place,
    ...
    for row in range(n_rows):
        ...
        for index in range(width):
            ...
            if kind == dialect.SHEET_CELL_DATE and dialect.sheet_iso_date(value):
                ...
                # (plan P4-D164), never as a string a reader hands back
                # as text.
