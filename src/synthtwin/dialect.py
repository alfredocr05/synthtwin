...
# A CELL STORED AS AN ISO DATE (`t="d"`, plan P4-D164). The file keeps
...
# class is handed only to a cell spelled as an error (plan P4-D162) --
...
# -- what a census of a workbook may publish (plan P4-D160) -----------
#
...
def _small(count: int, total: int, line: int) -> bool:
    ...
    if count <= 0 or count >= total:
        return False
    return count < line or total - count < line
...
def sheet_count(count: int, total: int, floor: int) -> "int | None":
    ...
    if total > 0 and count == total:
        return count
    if count >= line and total - count >= line:
        return count
    return None
...
#
# ONE READING, TWO CALLERS (plan P4-D162). The twin's writer hands a
...
def sheet_number_spelling(text: str) -> str:
    ...
    if place >= 0:
        ...
        # AN EXPONENT MARK OWES FIGURES (plan P4-D169): `1e` is text, and
...
def sheet_iso_date(text: str) -> bool:
    ...
    the shape a `t="d"` cell holds (plan P4-D164).
    """
    if not isinstance(text, str):
        raise TypeError("internal check: a cell's text was not text")
    clock = text
...
# HOW MANY FIGURES A PUBLISHED SHEET NAME MAY END IN (plan P4-D167). The
...
def twin_sheet_names(published: "tuple[str | None, ...]") -> "tuple[str, ...]":
    ...
    AND A NAME IS TAKEN WHATEVER ITS CASE (plan P4-D167). The published
...
def survey(
    ...
    #
    # AND AGAINST THE NAMES A HEADERLESS TABLE IS GIVEN (plan P4-D168). A
